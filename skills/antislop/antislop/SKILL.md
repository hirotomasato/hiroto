...
**Skip this section** if the project's entry file (`AGENTS.md`, `AGENTS.md`, `AGENTS.md`, or the equivalent the running tool reads at session start) already contains an antislop pointer block. That means antislop is already installed. Come back only when the user asks to add or remove a skill.
...
5. **Append the pointer block at the END of the project's entry file** (the file the running tool reads at session start: `AGENTS.md` for Hiroto, `AGENTS.md` for Hiroto, `AGENTS.md` for Hiroto, and so on). If that file does not exist, create it. Never modify existing content:
   ```md
   <!-- antislop:start -->
   ## antislop
...
- `AGENTS.md` (or `AGENTS.md`, `AGENTS.md`, etc.) routes the agent: "for UI work, read `DESIGN.md` for direction, then `antislop.md` as the filter."
